...
installcocextensions() { \
  ...
  sudo npm install coc-explorer coc-snippets coc-json coc-cmake coc-actions coc-texlab --global-style --ignore-scripts --no-bin-links --no-package-lock --only=prod
}

cloneconfig() { \
  echo "Cloning Nvim Mach 2 configuration"
  git clone https://github.com/th3rring/nvim.git ~/.config/nvim
}
